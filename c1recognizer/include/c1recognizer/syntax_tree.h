...
template <typename T>   //函数类模板
using ptr = std::shared_ptr<T>; //智能指针

// List of reference of type
template <typename T>
using ptr_list = std::vector<ptr<T>>;   //别名使用
...
struct cond_syntax : syntax_tree_node       //条件节点
{
    relop op;
...
struct expr_syntax : virtual syntax_tree_node //表达式节点
{
    virtual void accept(syntax_tree_visitor &visitor) = 0;
};
