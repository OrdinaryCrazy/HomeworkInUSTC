...
NeuralNetwork = MLPClassifier(hidden_layer_sizes=(500,300,100),activation='relu', solver='adam', alpha=0.0001, batch_size='auto', learning_rate='adaptive', learning_rate_init=0.001, power_t=0.5, max_iter=200, shuffle=True, random_state=None, tol=0.0001, verbose=False, warm_start=True, momentum=0.9, nesterovs_momentum=True, validation_fraction=0.20, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
...
for time in range(train_times):
    ...
    for index in train_file.index[time * bench_size:(time + 1) * bench_size]:
        ...
        tmp[LengthOfFeature + SentimentWordIn[train_file.sentiment_word[index]]] = 1
...
for time in range(train_times):
    ...
    predict_prob_y = NeuralNetworkForWord.predict_proba(test_x)
    test_auc = metrics.roc_auc_score(y_one_hot,predict_prob_y)
    print('time:%s\t'%time + "test_auc:\t" + str(test_auc))
