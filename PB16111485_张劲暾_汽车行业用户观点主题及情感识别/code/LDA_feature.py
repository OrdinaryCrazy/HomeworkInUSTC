...
#RFforTopic = RandomForestClassifier(n_estimators=200, random_state=1080)
#RFforSenti = RandomForestClassifier(n_estimators=200, random_state=1080)
...
#############################################################TransFormer = TfidfVectorizer(min_df=1e-4)
#print(CommentPlus)
#input()
...
#Xt = Xp[len(Comment):]
Result.subject = NeuralNetwork.predict(XTest)
Result.sentiment_value = NeuralNetForSenti.predict(XTest)
Result["sentiment_word"] = ''

Result.to_csv("./result_TF.csv")